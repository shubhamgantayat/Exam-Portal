...
class DB_Handler(Handler):
    ...
    def __init__(self, level=0, backup_logger_name=None):
        ...
        if backup_logger_name is not None:
            self.backup_logger_name = backup_logger_name
    ...
    def emit(self, record: LogRecord) -> None:
        try:
            ...
            client = pymongo.MongoClient(f"mongodb+srv://{username}:{password}@cluster0.qwvki.mongodb.net/myFirstDatabase?retryWrites=true&w=majority".format(username=username, password=password))
            my_db = client["ExamPortal"]
            record = {
            ...
            print(e)
            if self.backup_logger_name is not None:
